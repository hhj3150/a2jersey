...
이 앱은 **사전회원 + 마케팅 발송**만 담당합니다. 주문·결제·배송은 외부 공식 쇼핑몰이 담당합니다.

> **현재 전략**: 6/1부터 **아임웹 자체몰**을 공식 쇼핑몰로 운영. 5월 말 연결 예정.
> 향후 매출 규모·운영 부담에 따라 네이버 스마트스토어를 추가 채널로 검토.
> 공식 쇼핑몰 URL은 `VITE_SHOP_URL` env 한 줄로 교체 가능 (코드 수정 불필요).

```
┌────────────────────────┐    ┌──────────────────────────────────────────┐
│  공식 쇼핑몰            │    │  a2jersey (이 저장소)                     │
│  (아임웹 자체몰 →       │    │                                          │
│   추후 스마트스토어 등) │    │  공개 페이지 (Netlify)                   │
│                        │    │   • QR 랜딩, D-day, 사전회원 등록 폼     │
│  • 정기구독 가입       │    │   • SEO·OG 메타, FAQ, 공유                │
│  • 결제 / 환불         │    │                                          │
│  • 주문관리 / 발송처리 │    │  관리자 (/admin)                         │
│  • 택배사 연동         │    │   • 가입자 CRUD, 메모·상태               │
│  • CS / 정산           │    │   • CSV export                          │
...
공식 쇼핑몰(아임웹/스마트스토어 등)이 그 역할을 더 잘 합니다 (택배사 자동 연동, 결제 정산, CS 일원화).
이 앱의 admin은 "쇼핑몰이 못 하는 일" — 가입경로별 분석, 사전회원 마케팅 — 에만 집중합니다.

자체 배송 시스템이 정말 필요한 시점:
1. 월 매출이 일정 규모를 넘어 외부 쇼핑몰 수수료 부담이 커진 경우
2. 자사몰·마켓컬리·SSG 멀티채널 통합이 필요해진 경우
3. B2B 정기납품 (카페·호텔) 비중이 커진 경우
